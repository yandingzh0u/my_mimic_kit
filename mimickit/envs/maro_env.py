...
def resolve_maro_num_phases(config_value, motion_length, control_dt):
    ...
    if isinstance(config_value, numbers.Integral) and not isinstance(
        ...
        if num_phases <= 0:
            raise ValueError("maro_num_phases must be positive")
        return num_phases, False

    if config_value not in (None, "auto"):
        raise ValueError(
            "maro_num_phases must be a positive integer or 'auto'")
...
class MAROEnv(add_env.ADDEnv):
    ...
    for the learned differential reward.  It is available only to MARO's
    ...
    def __init__(self, env_config, engine_config, num_envs, device, visualize,
                 record_video=False):
        self._maro_num_phases_config = env_config.get(
            "maro_num_phases", "auto")
        super().__init__(
    ...
    def _load_motions(self, motion_file):
        ...
        if motion_lengths.numel() == 0:
            raise ValueError("MARO requires at least one reference motion")
        ...
        if not torch.isclose(max_length, min_length, atol=1e-6, rtol=1e-5):
            raise ValueError(
                "MARO currently requires equal-duration reference clips")

        control_dt = float(self._engine.get_timestep())
        if control_dt <= 0.0:
            raise ValueError("engine timestep must be positive")
        (self._maro_num_phases,
         self._maro_num_phases_is_auto) = resolve_maro_num_phases(
             self._maro_num_phases_config,
             float(max_length.item()), control_dt)
        return

    def get_maro_num_phases(self):
        return self._maro_num_phases

    def get_maro_num_phases_is_auto(self):
        return self._maro_num_phases_is_auto

    def _build_data_buffers(self):
        super()._build_data_buffers()
        self._maro_phase_idx_buf = torch.zeros(
            self.get_num_envs(), device=self._device, dtype=torch.long)
        self._info["maro_phase_idx"] = self._maro_phase_idx_buf
        return

    def _update_observations(self, env_ids=None):
        super()._update_observations(env_ids)
        if env_ids is None or len(env_ids) > 0:
            self._update_maro_phase_idx(env_ids)
        return

    def _update_maro_phase_idx(self, env_ids=None):
        if env_ids is None:
        ...
        phase_idx = phase_to_control_bin(phase, self._maro_num_phases)

        if env_ids is None:
            self._maro_phase_idx_buf.copy_(phase_idx)
        else:
            self._maro_phase_idx_buf[env_ids] = phase_idx
        return
