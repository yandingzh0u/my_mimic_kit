...
import learning.gain_sandwich_add_model as gain_sandwich_add_model
import util.torch_util as torch_util


class GainSandwichADDAgent(add_agent.ADDAgent):
    """Official ADD with a zero-anchored, self-scaled Sandwich discriminator."""
    def __init__(self, config, env, device):
        super().__init__(config, env, device)
        if self._disc_grad_penalty != 0:
            raise ValueError("Gain-Sandwich ADD requires disc_grad_penalty=0")

    def _build_model(self, config):
        self._model = gain_sandwich_add_model.GainSandwichADDModel(
    ...
    def _compute_disc_loss(self, batch):
        ...
        pos_logit = all_logits[:1]
        neg_logit = all_logits[1:]
        ...
        radius = torch.linalg.vector_norm(norm_diff, dim=-1) / math.sqrt(
            self._model.get_disc_input_dim())
        gap = torch.abs(neg_logit - pos_logit[0])
        gain = self._model.get_disc_gain()
        effective_radius = gain * radius
        violation = torch.relu(gap - effective_radius)
        ...
        return {
            ...
            "disc_rms_radius_mean": radius.mean().detach(),
            "disc_rms_radius_p99": torch.quantile(radius, 0.99).detach(),
            "disc_gap_mean": gap.mean().detach(),
            "disc_gap_p99": torch.quantile(gap, 0.99).detach(),
            "disc_gap_max": gap.max().detach(),
            "disc_lip_violation_max": violation.max().detach(),
            "disc_lip_violation_frac": (violation > 1e-4).float().mean().detach(),
            "disc_gain": gain.detach(),
            "disc_lipschitz_bound": self._model.get_disc_lipschitz_bound().detach(),
    ...
    def _calc_disc_rewards(self, norm_diff):
        with torch.no_grad(), self._model.cached_disc_transforms():
            logits = torch_util.eval_minibatch(
                self._model.eval_disc, {"diff": norm_diff},
                self._disc_eval_batch_size).squeeze(-1)
            return self._disc_reward_scale * add_agent.calc_unscaled_disc_reward(logits)
