...
class PLOTAgent(add_agent.ADDAgent):
    ...
    def __init__(self, config, env, device):
        ...
        self._plot_num_phases = env.get_plot_num_phases()
        self._plot_num_phases_is_auto = env.get_plot_num_phases_is_auto()
        return

    def _get_return_log_keys(self):
        # BaseAgent's online trackers see the environment diagnostic reward,
        # before PLOT constructs the reward sequence consumed by PPO.
        return ("Plot_Environment_Train_Return",
    ...
    def _record_data_post_step(self, next_obs, r, done, next_info):
        ...
        self._exp_buffer.record(
            "plot_phase_idx", next_info["plot_phase_idx"])
        return
    ...
    def _compute_rewards(self):
        ...
        phase_idx = self._exp_buffer.get_data_flat("plot_phase_idx")
        ...
        if not torch.all(torch.isfinite(weighted_disc_r)):
            raise FloatingPointError("PLOT produced non-finite policy rewards")
        ...
        info = {
            ...
            "plot_weighted_reward_mean": weighted_reward_mean,
            "plot_weighted_reward_std": weighted_reward_std,
            "plot_ppo_consumed_reward_mean": ppo_reward_mean,
            "plot_ppo_consumed_reward_std": ppo_reward_std,
            "plot_ppo_consumed_discounted_rollout_return_mean": (
                discounted_rollout_return_mean),
            "plot_ppo_consumed_discounted_rollout_return_std": (
                discounted_rollout_return_std),
            "plot_phase_utility_min": torch.min(observed_utility),
            "plot_phase_utility_mean": torch.mean(observed_utility),
            "plot_phase_utility_max": torch.max(observed_utility),
            "plot_phase_utility_softmin": softmin,
            "plot_phase_count_min": torch.min(phase_counts[present]),
            "plot_phase_count_max": torch.max(phase_counts[present]),
            "plot_phase_missing_fraction": 1.0 - torch.mean(present.float()),
            "plot_reward_weight_mean": torch.mean(phase_weights),
            "plot_reward_weight_std": torch.std(phase_weights),
            "plot_reward_weight_max": torch.max(phase_weights),
            "plot_probability_max": torch.max(observed_probability),
            "plot_probability_ratio": probability_ratio,
            "plot_adversarial_density_ratio": density_ratio,
            "plot_probability_entropy": entropy,
            "plot_probability_normalized_entropy": normalized_entropy,
            "plot_probability_effective_support": torch.exp(entropy),
