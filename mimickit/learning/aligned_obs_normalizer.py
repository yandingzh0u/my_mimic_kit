...
class AlignedObsNormalizer(torch.nn.Module):
    ...
    separate scale-only normalizer because it follows demonstration tangent
    statistics rather than policy-error statistics.
    """
    ...
    def set_error_normalizer(self, error_normalizer):
        if tuple(error_normalizer.get_shape()) != (self._command_dim,):
            raise ValueError("ADD error normalizer shape does not match command size")
    ...
    def normalize(self, obs):
        ...
        if self._error_norm_ref is None:
            raise RuntimeError("ADD error normalizer has not been attached")
