...
import envs.maro_env as maro_env  # noqa: E402
import envs.add_env as add_env  # noqa: E402
import learning.add_agent as add_agent  # noqa: E402
import learning.maro_agent as maro_agent  # noqa: E402
...
def test_phase_count_resolution_is_explicit_or_deterministic_half_up():
    ...
    for length in (1.75, 1.75 - 1e-8, 1.75 + 1e-8):
        automatic, automatic_is_auto = maro_env.resolve_maro_num_phases(
...
def test_invalid_phase_count_configuration_is_rejected(value):
    with pytest.raises(ValueError, match="maro_num_phases"):
        maro_env.resolve_maro_num_phases(
...
def test_phase_utility_uses_the_policy_reward_scale():
    ...
    utility = maro_agent.normalize_adversarial_reward(
...
def test_softmin_gradient_equals_phase_probability():
    ...
    beta = 1.9
    objective = maro_agent.compute_entropic_softmin(
        utility, occupancy, beta)
    objective.backward()
    probability = maro_agent.compute_phase_probabilities(
...
def test_occupancy_anchored_density_ratio_is_independent_of_phase_counts():
    ...
    density = maro_agent.compute_phase_density(utility, counts, beta)
...
def test_extreme_occupancy_does_not_amplify_equal_utility():
    ...
    density = maro_agent.compute_phase_density(utility, counts, beta=1.9)
...
def test_small_beta_recovers_standard_learned_reward():
    ...
    density = maro_agent.compute_phase_density(
...
def test_nonfinite_phase_utility_fails_fast():
    with pytest.raises(ValueError, match="phase_utility must be finite"):
        maro_agent.compute_phase_density(
...
def test_discounted_rollout_return_uses_the_exact_ppo_reward_sequence():
    ...
    result = maro_agent.compute_discounted_rollout_return(
        reward, discount=0.5)
    assert torch.allclose(
...
def test_phase_argmax_log_uses_paper_one_based_indexing():
    ...
    assert maro_agent.one_based_phase_argmax(probability).item() == 2


def test_maro_renames_online_environment_return_logs():
    assert maro_agent.MAROAgent._get_return_log_keys(None) == (
        "Maro_Environment_Train_Return",
        "Maro_Environment_Test_Return",
    )


def test_maro_keeps_the_learned_differential_discriminator():
    assert maro_agent.MAROAgent.__bases__ == (add_agent.ADDAgent,)
    assert maro_env.MAROEnv.__bases__ == (add_env.ADDEnv,)
    assert "_compute_disc_loss" not in maro_agent.MAROAgent.__dict__
    assert "_build_model" not in maro_agent.MAROAgent.__dict__


def test_public_maro_configuration_names():
    agent = yaml.safe_load(
        (ROOT / "data/agents/maro_humanoid_agent.yaml").read_text())
    assert agent["agent_name"] == "MARO"
    assert agent["disc_grad_penalty"] == 2
    assert agent["maro_beta"] == pytest.approx(1.90022564)
    assert agent["maro_phase_prior"] == "occupancy"
    ...
    for motion, num_phases in expected_phases.items():
        env = yaml.safe_load(
            (ROOT / "data/envs/paper_benchmark"
             / f"maro_{motion}_env.yaml").read_text())
        assert env["env_name"] == "maro"
        ...
        assert env["maro_num_phases"] == num_phases
