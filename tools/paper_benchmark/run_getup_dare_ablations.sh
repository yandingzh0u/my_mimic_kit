...
# Serial 2x2 ablation campaign for DARE on Get-Up (facedown).
#
# The existing Full DARE controller is the (+ group, + calibration) cell:
#   output/paper_benchmark/dare_getup_facedown_2k_8192_seed0
...
usage() {
  printf '%s\n' \
    ...
    "" \
    "Runs the three missing Get-Up DARE ablations serially using env_isaaclab."
}
...
readonly motion="getup_facedown"
readonly seed=0
readonly env_config="data/envs/paper_benchmark/dare_getup_facedown_env.yaml"
readonly engine_config="data/engines/isaac_lab_engine.yaml"
readonly args_file="args/paper_benchmark/dare_2k_8192_args.txt"
readonly full_dare_output="output/paper_benchmark/dare_getup_facedown_2k_8192_seed0"

variants=(wogroup wocalibration base)
declare -A agent_configs=(
  [wogroup]="data/agents/ablations/dare_getup_wogroup_agent.yaml"
  [wocalibration]="data/agents/ablations/dare_getup_wocalibration_agent.yaml"
  [base]="data/agents/ablations/dare_getup_base_agent.yaml"
)
...
readonly plan_file="$campaign_root/getup_dare_ablation_plan.tsv"
readonly events_file="$campaign_root/getup_dare_ablation_events.tsv"
readonly launcher_log="$campaign_root/getup_dare_ablation_launcher.log"
...
exec 9>"$campaign_root/.getup_dare_ablation.lock"
if ! flock -n 9; then
  printf 'ERROR: another Get-Up DARE ablation launcher holds the campaign lock\n' >&2
  exit 3
fi
...
write_plan() {
  ...
  for variant in "${variants[@]}"; do
    ...
    printf '%d\t%s\t%s\t%s\t%s\t%s\tpending\n' \
      ...
      "$campaign_root/getup_facedown_${variant}_2k_8192_seed${seed}" \
      >> "$plan_file"
  done
}
...
run_job() {
  ...
  local out_dir="$root/getup_facedown_${variant}_${suffix}_seed${seed}"
...
printf '[%s] Get-Up DARE ablation campaign complete.\n' "$(date --iso-8601=seconds)"
