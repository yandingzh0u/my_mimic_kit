...
# Restartable launcher for the 8192-environment, 2000-iteration Roll run.
# It refuses concurrent MimicKit training, records an immutable source/config
# manifest, resumes only a structurally complete checkpoint, and marks DONE
# only after the common behavior evaluator has produced every artifact.

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$repo_root"

python_bin="${MIMICKIT_PYTHON:-/home/y/miniconda3/envs/env_isaaclab/bin/python}"
arg_file="args/anchored_energy_humanoid_roll_2k_8192_args.txt"
agent_file="data/agents/anchored_energy_add_humanoid_agent.yaml"
env_file="data/envs/anchored_energy_humanoid_roll_env.yaml"
engine_file="data/engines/isaac_lab_engine.yaml"
out_dir="output/anchored_energy_roll_2k_8192_seed0"
...
if ! flock -n 9; then
  printf 'ERROR: another launcher holds %s\n' "$out_dir/launcher.lock" >&2
  exit 2
fi
...
if [[ -s "$manifest" ]]; then
  # Values are generated only by git, sha256sum, hostname, and date below.
  # shellcheck disable=SC1090
  source "$manifest"
  [[ "${MANIFEST_GIT_SHA:-}" == "$git_sha" ]] || {
    printf 'ERROR: Git commit differs from the original manifest.\n' >&2
    exit 5
  }
...
if metadata["agent_class"] != "AnchoredEnergyADDAgent":
...
required_model_keys = {
    ...
    "_energy_ref_scale",
    "_model._disc_beta",
    "_model._conditional_energy._context_layers.0.weight",
    "_model._conditional_energy._metric_head.weight",
}
if not required_model_keys.issubset(checkpoint["model_state_dict"]):
    raise SystemExit("checkpoint anchored-energy state is incomplete")
if set(checkpoint["replay_buffer_states"]["_disc_buffer"]["buffers"]) != {
        "disc_obs", "disc_obs_demo", "ref_motion"}:
    raise SystemExit("checkpoint paired replay state is incomplete")
...
if [[ -s "$checkpoint_file" ]]; then
  ...
  if (( samples < target_samples )); then
    ...
    printf 'Training budget already reached at %s samples; evaluating.\n' \
      "$samples"
  fi
elif [[ -e "$out_dir/model.pt" || -e "$out_dir/log.txt" ]]; then
  printf 'ERROR: refusing to overwrite artifacts without a checkpoint.\n' >&2
  exit 7
fi
